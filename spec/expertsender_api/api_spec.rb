...
describe ExpertSenderApi::API do
  ...
  context 'when configured properly' do
    ...
    describe '##add_subscribers_to_list calls post with correct body' do
      it do
    ...
    describe '##remove_subscriber_from_list by id calls delete with correct parameters' do
      it do
    ...
    describe '##remove_subscriber_from_list by email returns success response' do
      it do
        expected_params = { apiKey: api_key,
    ...
    describe '##get_subscriber_info calls get with correct parameters' do
      it do
        expected_params = { apiKey: api_key,
                            email: subscriber_attributes[:email],
    ...
    describe '##create_and_send_email calls post with correct body' do
      it do
        builder = Nokogiri::XML::Builder.new do |xml|
          xml.ApiRequest(ExpertSenderApi::API::XML_NAMESPACES) do
            xml.ApiKey api_key
    ...
    describe '##send_transaction_email' do
      it do
        letter_id = 93
    ...
    describe '##get_deleted_subscribers calls get with correct parameters' do
      it do
        expected_params = { apiKey: api_key,
                            listIds: '52,53',
        ...
        subject.get_deleted_subscribers(list_ids: [52, 53],
                                        remove_types: %w(OptOutLink Compliant Ui),
                                        start_date: Date.today,
                                        end_date: Date.new(2090, 1, 1))
      end
    end

    describe '##add multiple data to data table call' do
      it do
        builder = Nokogiri::XML::Builder.new do |xml|
          xml.ApiRequest(ExpertSenderApi::API::XML_NAMESPACES) do
            xml.ApiKey api_key
    ...
    describe '##clear data table calls' do
      it do
        builder = Nokogiri::XML::Builder.new do |xml|
          xml.ApiRequest(ExpertSenderApi::API::XML_NAMESPACES) do
            xml.ApiKey api_key
  ...
  context 'when has wrong api key' do
    ...
    describe '##add_subscribers_to_list raises exception' do
      it do
  ...
  def expect_get(expected_url, expected_params)
    expect(ExpertSenderApi::API).to receive(:get).with(expected_url, {query: expected_params}).and_return(Struct.new(:body).new(nil))
  end

  def expect_post(expected_url, expected_body)
    expect(ExpertSenderApi::API).to receive(:post).with(expected_url, {body: expected_body}).and_return(Struct.new(:body).new(nil))
  end

  def expect_delete(expected_url, expected_params)
    expect(ExpertSenderApi::API).to receive(:delete).with(expected_url, {query: expected_params}).and_return(Struct.new(:body).new(nil))
  end
end
